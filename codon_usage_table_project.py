...
def pull_filelists_ncbi():  # Retrieve listings of files from NCBI's servers, from specified divisions of RefSeq/GenBank 
...
def create_all_requests():  # create all the requests that handle the data processing
    start = 0
    index = 0
    cur_weight = 0
    progress_timer = time.time()
...
def create_request(division, start, end, rn):  # create individual request
...
def get_filelist(): # Get the files this individual request needs to parse by looking at the lines of the overall filelisting
...
def print_exception_source(assembly, accession, protein_id): # If a sequence fails, print the information about that sequence 
...
def refseq_check_file_downloads(): # attempt to download relevant files only if they're been updated or you don't have a local copy 
...
def genbank_check_file_downloads():  # same as refseq's function but for genbank's structure
...
class SpeciesCodons(object):
    def __init__(self, division, assembly, taxid, species, organelle):
        ...
        self.organelle = organelle  # genomic, mitochondria, chloroplast, chromoplast, leucoplast
        self.codons = CodonsDict.copy()
        self.transl_table = []  # list in case there's more than one value per organism
...
class CDS_Codons(object): # mostly same as species, but for individual coding sequences
...
def refseq_check_total_CDS(refseq_filelist): # count the total number of CDSes in request's files (for accurate progress reporting)
    progress_timer = time.time()
    total_num_CDS = 0
...
def genbank_check_total_CDS(genbank_filelist):  # count the total number of CDSes in request's files (for accurate progress reporting) 
    progress_timer = time.time()
    total_num_CDS = 0
...
def combine_individual_outputs():  # combine the output from individual requests into their GB/RS level files
    progress_timer = time.time()
...
def launch_extend_codon_table(): # special function to launch combine function if last request to finish
...
def waiting_for_extend_codon_table():
    extend_codon_table_req = launched_extend_codon_table()
    if extend_codon_table_req:
        status = pyhive.proc.req_get_status(extend_codon_table_req)
        return status > 0 and status < pyhive.req_status.DONE
    else:
        return False
    

def on_execute(req_id):  # the main function to be run
